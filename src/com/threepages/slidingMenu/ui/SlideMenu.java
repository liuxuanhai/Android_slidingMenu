package com.threepages.slidingMenu.ui;
...
public class SlideMenu extends ViewGroup {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (newScrollPosition < -getChildAt(0).getMeasuredWidth()){ // 限定左边距
				// < -240
				scrollTo(-getChildAt(0).getMeasuredWidth(), 0);
			} else if (newScrollPosition > 0){ // 限定右边距
				// > 0
				scrollTo(0, 0);
			} else {
				// 让变化量生效
...
//		- 201 -> -202 -> -203 .... -236 -> -240
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if(xOffset > yOffset && xOffset > 5){ // 水平方向超出
